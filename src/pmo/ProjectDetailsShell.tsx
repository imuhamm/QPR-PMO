...
import { mockActiveProjectMeta, mockProjectMeta, mockSectionReadiness } from './data/mockProjectDetails'
...
import { cascadeReschedule, initialScheduleRows, MOCK_CURRENT_USER, MOCK_TODAY } from './schedule/scheduleData'
...
export function ProjectDetailsShell({ projectId, onBack }: { projectId?: string; onBack?: () => void }) {
  // Only proj-core-banking-migration has moved into execution — every other
  // id (including an unset one) falls back to the Draft demo workspace, see
  // App.tsx. Lifted into state (not a plain const) because Update Progress
  // Mode's save needs to move execution.lastUpdated forward.
  const [projectMeta, setProjectMeta] = useState<ProjectMeta>(
    projectId === 'proj-core-banking-migration' ? mockActiveProjectMeta : mockProjectMeta,
  )
